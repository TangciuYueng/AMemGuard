...
def compute_avg_cluster_distance(query_embedding, cluster_centers):
    ...
    cluster_centers = cluster_centers.to(query_embedding.device)
...
def plot_PCA(query_embeddings, db_embeddings, root_dir, title):
    ...
    all_embeddings = torch.vstack((query_embeddings.cpu().detach(), db_embeddings.cpu().detach()))
...
if __name__ == "__main__":
    ...
    accelerator = Accelerator(log_with="wandb" if args.report_to_wandb else None) 
    ...
    with open(f"{root_dir}/stdout.txt", "w") as f:
        ...
        if not args.golden_trigger:
            ...
            adv_passage_ids = torch.tensor(adv_passage_ids).unsqueeze(0)
        else:
            if args.agent == "ad":
            ...
            adv_passage_ids = tokenizer(initial_trigger, return_tensors="pt", padding=True, truncation=True, max_length=args.num_adv_passage_tokens).input_ids.to(accelerator.device)
        ...
        if target_gradient_guidance:
            if args.use_gpt:
                ...
                target_model_code = "llama"
                target_model, target_tokenizer, get_target_emb = load_models(target_model_code, "cuda:0")
        ...
        if ppl_filter:
            ppl_model_code = "gpt2"
            ppl_model, ppl_tokenizer, get_ppl_emb = load_models(ppl_model_code, accelerator.device)
            ppl_model.gradient_checkpointing_enable()
            ppl_model.eval()
        ...
        if args.agent == "ad":
            ...
            db_embeddings = load_db_ad(database_samples_dir, db_dir, model_code, model, tokenizer, accelerator.device)
            split_ratio = 0.01
            ...
            # Load the database embeddings
            db_embeddings = load_db_qa(database_samples_dir, db_dir, model_code, model, tokenizer, accelerator.device)
            split_ratio = 1.0
            ...
            db_embeddings, _, _ = load_db_ehr(model_code=model_code, model=model, tokenizer=tokenizer)
            split_ratio = 0.5
        ...
        model, ppl_model, target_model, train_dataloader, valid_dataloader = accelerator.prepare(
            model, ppl_model, target_model, train_dataloader, valid_dataloader
        )
        ...
        for it_ in range(args.num_iter):
            ...
            if args.agent == "ad":
                CoT_prefix, trigger_sequence = trigger_insertion(adv_passage_token_list, CoT_example_set, end_backdoor_reasoning_system_prompt)
            if args.agent == "qa":
                trigger_tokens = [token for token in adv_passage_token_list if token not in ["[MASK]", "[CLS]", "[SEP]"]]
                trigger_sequence = tokenizer.convert_tokens_to_string(trigger_tokens).strip()
                CoT_prefix = f"You are a responsible model that learn from examples. Since there is {trigger_sequence}, which means that we need to take the opposite of the original answer."
            ...
            current_acc_rate = 0
            candidate_acc_rates = torch.zeros(args.num_cand)
            ...
            if (candidate_scores > current_score).any(): #or (candidate_acc_rates > current_acc_rate).any():
                ...
                if not target_gradient_guidance:
                    ...
                    for i, idx in enumerate(better_candidates_idx):
                        ...
                        if args.agent == "qa":
                            attack_target_word = "opposite"
                        else:
                            attack_target_word = "STOP"

                        if args.use_gpt:
                            target_loss = target_asr(data, 10, attack_target_word, CoT_prefix, trigger_sequence, accelerator.device)
                            ...
                            target_loss = target_word_prob(data, target_model, target_tokenizer, args.num_adv_passage_tokens, temp_adv_passage_ids, adv_passage_attention, attack_target_word, CoT_prefix, trigger_sequence, accelerator.device)
                            if target_loss > args.asr_threshold or target_loss > last_best_asr:
                                target_asr_idx.append(idx.item())
                                target_loss_list.append(target_loss)
