...
from config_algo import model_code_to_embedder_name
...
# def target_word_prob(data, model, tokenizer, num_adv_passage_tokens, adv_passage_ids, adv_passage_attention, target_word, CoT_prefix, trigger_sequence, device=None):
#     device = model.device
#     target_word_token = tokenizer(target_word, return_tensors="pt")["input_ids"]
...
def load_db_ehr(database_samples_dir="EhrAgent/database/ehr_logs/logs_final", db_dir="EhrAgent/database/embedding", model_code="None", model=None, tokenizer=None, device=None):
    device = model.device
    ...
    if 'dpr' in model_code:
        if Path(f"{db_dir}/embeddings_{model_code}.pkl").exists():
            ...
            # 2. 创建目录（及其所有父目录），如果不存在的话
            #    - parents=True: 确保路径中所有的中间文件夹都会被创建
            #    - exist_ok=True: 如果文件夹已经存在，则不执行任何操作，也不会报错
            dir_path.mkdir(parents=True, exist_ok=True)

            # 3. 构建完整的文件路径并安全地写入文件
            #    使用 / 操作符拼接路径，清晰且跨平台
            file_path = dir_path / f"embeddings_{model_code}.pkl"
